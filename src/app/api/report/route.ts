...
async function buildPDF(data: ReportInput): Promise<Uint8Array> {
  ...
  drawText(p1, "InstantLoan", 36, H - 42, bold, 22, WHITE);
  ...
  drawText(p1, "InstantLoan AI Report — Confidential · Not for circulation", 36, 13, normal, 8, MUTED);
  ...
  drawText(p2, "InstantLoan", 36, H - 24, bold, 14, WHITE);
  ...
  if (cy > 80) {
    ...
    const disc2 = "the bank's internal credit assessment. InstantLoan does not guarantee approval. All figures are estimates.";
  ...
  try {
    ...
    return new NextResponse(Buffer.from(pdf), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="InstantLoan_Report_${data.user.name?.replace(/\s+/g, "_") || "Report"}.pdf"`,
      },
    });
  } catch (err) {
